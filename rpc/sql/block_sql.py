...
def create_table(conn, mainnet_id, start_height):
    cursor = conn.cursor()
    table_name = "mainnet_%d" % mainnet_id
    sql = f"CREATE TABLE {table_name} ("\
        f"Height INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,"\
        f"ID CHAR(64) UNIQUE KEY NOT NULL,"\
        f"ParentID CHAR(64) NOT NULL,"\
        f"Timestamp VARCHAR(20) NOT NULL,"\
        f"SealNum SMALLINT UNSIGNED NOT NULL,"\
        f"CollectionNum SMALLINT UNSIGNED NOT NULL,"\
        f"Collections TEXT"\
        f") AUTO_INCREMENT={start_height}"
...
def create_table_all_in_one(conn, start_height):
    cursor = conn.cursor()
...
def insert_table(conn, data, table_name):
    ...
    for entry in data:
        sql = f"INSERT INTO {table_name} "\
            f"(Height, ID, ParentID, Timestamp, SealNum, CollectionNum, Collections) "\
            f"VALUES "\
            f"("\
